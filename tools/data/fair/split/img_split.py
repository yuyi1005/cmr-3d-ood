...
def add_parser(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=[1.],
        help='same as FAIR devkit rate, but only change windows size')
    parser.add_argument(
        '--img-rate-thr',
        type=float,
        default=0.6,
...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
    ...
    for i in range(windows.shape[0]):
        ...
        outdir = os.path.join(anno_dir, patch_info['id'] + '.xml')
        ...
        for idx in range(bboxes_num):
            bb = patch_info['ann']['bboxes'][idx]
            cn = patch_info['ann']['labels'][idx]
...
def load_fair(img_dir, ann_dir=None, nproc=10):
    """Load FAIR dataset.
    ...
    print('Starting loading FAIR dataset information.')
    start_time = time.time()
    _load_func = partial(_load_fair_single, img_dir=img_dir, ann_dir=ann_dir)
    if nproc > 1:
        pool = Pool(nproc)
        contents = pool.map(
            _load_func,
            filter(lambda x: int(osp.splitext(x)[0]) < 8137,
                   os.listdir(img_dir)))
        pool.close()
    else:
    ...
    end_time = time.time()
    print(f'Finishing loading FAIR, get {len(contents)} iamges,',
          f'using {end_time - start_time:.3f}s.')

    return contents


def _load_fair_single(imgfile, img_dir, ann_dir):
    """Load FAIR's single image.
    ...
    content = _load_fair_xml(xmlfile)
...
def _load_fair_xml(xmlfile):
    """Load FAIR's txt annotation.
...
def main():
    ...
    for img_dir, ann_dir in zip(args.img_dirs, args.ann_dirs):
        _infos = load_fair(img_dir=img_dir, ann_dir=ann_dir, nproc=args.nproc)
