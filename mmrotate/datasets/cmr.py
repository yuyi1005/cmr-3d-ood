...
class CMRDataset(BaseDataset):
    """CMR dataset for detection.

    Note: ``ann_file`` in CMRDataset is different from the BaseDataset.
    In BaseDataset, it is the path of an annotation file. In CMR Dataset,
            ...
            for txt_file in txt_files:
                ...
                with open(txt_file) as f:
                    ...
                    for si in s:
                        instance = {}
                        bbox_info = si.split()
                        instance['bbox'] = [float(i) for i in bbox_info[:5]]
                        instance['bbox'][4] *= 3.14159265 / 180
                        cls_name = 'Heart'
                        instance['bbox_label'] = [cls_map[cls_name], float(bbox_info[7]) * 1e5]
