...
def draw_3d_arrows(image, rboxes, arrows):
    img = vedo.Image(image[::-1])

    arrows_obj = []
    for rbox, arrow in zip(rboxes, arrows):
        center = np.array([rbox[0], rbox[1], 0])
        arrow = np.clip(arrow, 0.0, 1.0)
        k = np.clip((1 - arrow**2), 0.0, 1.0)**0.5
        vector = np.array([np.cos(rbox[4]) * k, np.sin(rbox[4]) * k, arrow])
        arrows_obj.append(vedo.Arrow(center, center + 50 * vector, s=0.7))

    plt = vedo.show(img, *arrows_obj, offscreen=True)
    plt.camera.OrthogonalizeViewUp()
    vis = plt.screenshot(asarray=True)
    vis = vis[vis.shape[0] // 2 + 324:vis.shape[0] // 2 - 324:-1, vis.shape[1] // 2 - 324:vis.shape[1] // 2 + 324]
...
class RotLocalVisualizer(DetLocalVisualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'bboxes' in instances:
            ...
            if labels.dim() == 2:
                instances.arrows = labels[:, 1] / 1e5
                labels = labels[:, 0]

            max_label = int(max(labels) if len(labels) > 0 else 0)
            # max_label = 0
            text_palette = get_palette(self.text_color, max_label + 1)
            # text_colors = [text_palette[0] for label in labels]
            text_colors = [text_palette[label] for label in labels]

            bbox_color = palette if self.bbox_color is None \
                else self.bbox_color
            bbox_palette = get_palette(bbox_color, max_label + 1)
            # colors = [bbox_palette[0] for label in labels]
        ...
        if 'arrows' in instances:
            bboxes = instances.bboxes
            arrows = instances.arrows
            self.set_image(draw_3d_arrows(self.get_image(), bboxes, arrows))
