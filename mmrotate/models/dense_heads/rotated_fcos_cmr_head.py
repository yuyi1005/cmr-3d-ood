...
class RotatedFCOSCMRHead(FCOSHead):
    ...
    def __init__(self,
                 ...
                 use_cosine_similarity_loss: bool = False,
                 **kwargs):
    ...
    def loss_by_feat(
        ...
        flatten_arrow_targets = flatten_labels[:, 1] / 1e5
        ...
        pos_arrow_targets = flatten_arrow_targets[pos_inds]
        ...
        if len(pos_inds) > 0:
            ...
            if self.loss_angle is not None:
                t = torch.cat([pos_angle_targets, pos_arrow_targets[:, None]], dim=-1)
                pos_angle_targets = self.angle_coder.encode(t)
                loss_angle = self.loss_angle(
                    pos_angle_preds, pos_angle_targets, avg_factor=num_pos)
                if self.use_cosine_similarity_loss:
                    u = self.angle_coder.decode_to_xyz(pos_angle_preds)
                    v = self.angle_coder.decode_to_xyz(pos_angle_targets)
                    cos_theta = torch.nn.functional.cosine_similarity(u, v, dim=1).clamp(-1.0, 1.0)
                    loss_angle += self.loss_angle(cos_theta, torch.ones_like(cos_theta), avg_factor=num_pos)
    ...
    def _predict_by_feat_single(self,
        ...
        for level_idx, (
            ...
            decoded_angle = self.angle_coder.decode(angle_pred)
        ...
        bboxes = self.bbox_coder.decode(priors, bbox_pred[:, :5], max_shape=img_shape)

        results = InstanceData()
        results.bboxes = RotatedBoxes(bboxes)
        results.arrows = bbox_pred[:, 5]
        # results.arrows = torch.sqrt(1 - bbox_pred[:, 5]**2)
