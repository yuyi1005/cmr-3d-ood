...
class DeltaXYWHPSCHBBoxCoder(BaseBBoxCoder):
    ...
    def __init__(self,
                 target_means: Sequence[float] = (0., 0., 0., 0., 0., 0., 0.),
                 target_stds: Sequence[float] = (1., 1., 1., 1., 1., 1., 1.),
...
def bbox2delta(proposals: HorizontalBoxes,
    ...
    d0 = torch.cos(2 * torch.pi * 0 / 3 + gt * 2)
    d1 = torch.cos(2 * torch.pi * 1 / 3 + gt * 2)
    d2 = torch.cos(2 * torch.pi * 2 / 3 + gt * 2)
    deltas = torch.stack([dx, dy, dw, dh, d0, d1, d2], dim=-1)
...
def delta2bbox(rois: Tensor,
               deltas: Tensor,
               means: Sequence[float] = (0., 0., 0., 0., 0., 0., 0.),
               stds: Sequence[float] = (1., 1., 1., 1., 1., 1., 1.),
    ...
    reshaped_deltas = deltas.view(delta_shape[:-1] + (-1, 7))
    ...
    dw = denorm_deltas[..., 2]
    dh = denorm_deltas[..., 3]
    dt = denorm_deltas[..., 4:7]

    coef_sin = torch.sin(2 * torch.pi * dt.new_tensor((0, 1, 2)) / 3)
    coef_cos = torch.cos(2 * torch.pi * dt.new_tensor((0, 1, 2)) / 3)
    phase_sin = torch.sum(dt * coef_sin, dim=-1)
    phase_cos = torch.sum(dt * coef_cos, dim=-1)
    phase = -torch.atan2(phase_sin, phase_cos)  # In range [-pi,pi)
    gt = phase / 2

    # Compute center of each roi
    px = ((rois[..., None, 0] + rois[..., None, 2]) * 0.5)
    py = ((rois[..., None, 1] + rois[..., None, 3]) * 0.5)
    # Compute width/height of each roi
    pw = (rois[..., None, 2] - rois[..., None, 0])
    ph = (rois[..., None, 3] - rois[..., None, 1])
