...
class PSCCoder(BaseBBoxCoder):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.angle_version = angle_version
        assert angle_version in ['le90', 'r360']
        self.omega = omega
    ...
    def encode(self, angle_targets: Tensor) -> Tensor:
        ...
        """
        phase_targets = angle_targets * self.omega
        ...
        if self.dual_freq:
            phase_targets = angle_targets * self.omega * 2
            phase_shift_targets += tuple(
                torch.cos(phase_targets + 2 * torch.pi * x / self.num_step)
                for x in range(self.num_step))
...
@TASK_UTILS.register_module()
class PseudoAngle3DCoder(BaseBBoxCoder):
    """Pseudo Angle Coder."""
...
@TASK_UTILS.register_module()
class PSC3DCoder(BaseBBoxCoder):
    """Phase-Shifting Coder.
    ...
    def __init__(self,
        ...
        self.omega = omega
        self.num_step = num_step
        self.encode_size = self.num_step + 1
    ...
    def encode(self, angle_targets: Tensor) -> Tensor:
        ...
        z = angle_targets[..., 1:2]
        z = z.clamp(-1.0, 1.0)
        k = torch.sqrt(2 / self.num_step * (1 - z**2).clamp(min=0.0))

        phase_targets = angle_targets[..., 0:1] * self.omega
        if self.num_step == 2:
            x = torch.cos(phase_targets)
            y = torch.sin(phase_targets)
            phase_shift_targets = (k * x, k * y)
        else:
            phase_shift_targets = tuple(
                k * torch.cos(phase_targets + 2 * torch.pi * x / self.num_step)
                for x in range(self.num_step))

        return torch.cat(phase_shift_targets + (z,), axis=-1)
    ...
    def decode(self, angle_preds: Tensor) -> Tensor:
        ...
        """
        if self.num_step == 2:
            x = angle_preds[..., 0:1]          # k*cos(theta)
            y = angle_preds[..., 1:2]         # k*sin(theta)
            phase = torch.atan2(y, x)          # range (-pi, pi]
        else:
            self.coef_sin = self.coef_sin.to(angle_preds)
            self.coef_cos = self.coef_cos.to(angle_preds)
            phase_sin = torch.sum(
        ...
        z = angle_preds[..., -1:]
        theta = phase / self.omega
        return torch.cat([theta, z], axis=-1)
    
    def decode_to_xyz(self, angle_preds: Tensor) -> Tensor:
        p = angle_preds[..., 0:self.num_step]   # (..., N)
        z = angle_preds[..., -1:]               # (..., 1)
        ...
        if self.num_step == 2:
            ...
            self.coef_sin = self.coef_sin.to(angle_preds)
            self.coef_cos = self.coef_cos.to(angle_preds)
            scale = 2.0 / float(self.num_step)
            x = scale * torch.sum(p * self.coef_cos, dim=-1, keepdim=True)
            y = -scale * torch.sum(p * self.coef_sin, dim=-1, keepdim=True)

        return torch.cat([x, y, z], dim=-1)


@TASK_UTILS.register_module()
class XYZ3DCoder(BaseBBoxCoder):
    """Phase-Shifting Coder.
    ...
    def encode(self, angle_targets: Tensor) -> Tensor:
        ...
        Returns:
            list[Tensor]: The psc coded data (phase-shifting patterns)
                ...
                Has shape (num_anchors * H * W, encode_size)
        """
        x = torch.cos(angle_targets[..., 0:1])
        y = torch.sin(angle_targets[..., 0:1])
        z = angle_targets[..., 1:2]
        z = z.clamp(-1.0, 1.0)
        k = torch.sqrt((1 - z**2).clamp(min=0.0))
        return torch.cat([k * x, k * y, z], axis=-1)
    ...
    def decode(self, angle_preds: Tensor) -> Tensor:
        ...
        Returns:
            list[Tensor]: Angle offset for each scale level.
                Has shape (num_anchors * H * W, 2)
        """
        angle_preds = torch.nn.functional.normalize(angle_preds, p=2, dim=-1)
        x = angle_preds[..., 0:1]          # cos(theta)
        y = angle_preds[..., 1:2]         # since cos(theta+pi) = -cos(theta)
        theta = torch.atan2(y, x)          # range (-pi, pi]
        z = angle_preds[..., 2:3]
        return torch.cat([theta, z], dim=-1)
